...
function openImageModal(imageUrl, dashboardName, description, creatorName, creatorAvatar, categoriesText, createdAt, updatedAt, faqUrl = '', faqTitle = '') {
  ...
  const faqWrap = document.getElementById("modalFaqWrap");
  const faqLink = document.getElementById("modalFaqLink");
  ...
  if (faqUrl && faqUrl !== "#") {
    faqLink.href = faqUrl;
    faqLink.innerHTML = `<i class="fa-solid fa-book-open"></i> Ver Documentação: ${faqTitle || 'FAQ'}`;
  ...
  viewButtons.forEach((button) => {
    button.addEventListener("click", function (e) {
      ...
      const dashboardName =
        ...
        "Dashboard";
      const faqUrl = this.getAttribute("data-faq-url") || "";
...
function renderBulkPreview() {
    const grid = document.getElementById('bulkPairsGrid');
    const pairCountEl = document.getElementById('bulkPairCount');
    if (pairCountEl) {
        pairCountEl.textContent = `${bulkDashboardPairs.length} Dashboard(s)`;
    }
    grid.innerHTML = '';
    ...
    bulkDashboardPairs.forEach((pair, index) => {
        ...
        const faqOptionsHtml = ALL_FAQS.map((faq) => {
            return `<option value="${faq.id}" ${pair.faq === String(faq.id) ? 'selected' : ''}>${escapeHtml(faq.title)}</option>`;
        }).join('');
        ...
        card.innerHTML = `
            ...
            <div class="bulk-pair-info">
                ...
                <div class="bulk-pair-control">
                    <label for="bulk-faq-${pair.id}" style="font-weight: 600;">Vincular FAQ obrigatória</label>
                    <select id="bulk-faq-${pair.id}" class="form-input bulk-faq-select" onchange="updateBulkFaq('${pair.id}', this.value)" style="font-size: 0.9rem; padding: 0.5rem; margin-top: 0.35rem; width: 100%;">
...
window.updateBulkFaq = function(pairId, value) {
    const pair = bulkDashboardPairs.find(p => p.id === pairId);
    if (pair) {
        pair.faq = value;
        renderBulkPreview();
...
async function submitBulkDashboards() {
    ...
    const missingFaq = bulkDashboardPairs.some((pair) => !pair.faq);
    if (missingFaq) {
        alert('Selecione uma FAQ para todos os dashboards antes de confirmar.');
        confirmBtn.disabled = false;
        confirmBtn.innerHTML = 'Confirmar e Salvar Dashboards';
        return;
    }

    const missingImage = bulkDashboardPairs.some((pair) => !pair.imageFile);
    if (missingImage) {
        alert('Cada dashboard precisa de imagem para criação. Corrija os pares sem imagem antes de enviar.');
        confirmBtn.disabled = false;
        confirmBtn.innerHTML = 'Confirmar e Salvar Dashboards';
        return;
    }
    ...
    bulkDashboardPairs.forEach((pair, i) => {
        ...
        formData.append(`faq_${i}`, pair.faq);
...
dashboards.forEach(card => {
    ...
    // Pega o título (tenta H4, se não houver, tenta H3)
    const title = card.querySelector('h4, h3')?.textContent.trim() || 'Dashboard';
    
    // O nome do cliente pode não existir em cards de páginas específicas de cliente
    const clientNameElem = card.querySelector('.dashboard-client');
    const clientName = clientNameElem ? ` <small style="color: var(--app-muted); font-weight: normal; margin-left: 4px;">(${clientNameElem.textContent.trim()})</small>` : '';
    
    // BUSCA DE LINKS - Mais flexível
    // Procura qualquer elemento que tenha o atributo de preview, não só pela classe
    const viewBtn = card.querySelector('[data-preview-src]'); 
    ...
    // Procura o link de download (pela classe ou pelo ícone dentro)
    const downloadBtn = card.querySelector('.btn-download-dash, a[title*="Download"]');
    const downloadUrl = (downloadBtn && downloadBtn.getAttribute('href')) ? downloadBtn.getAttribute('href') : '#';
    ...
    item.innerHTML = `
        <div class="fqi-main" style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
            <div style="display: flex; align-items: center; flex: 1; overflow: hidden; padding-right: 10px;">
                ...
                <span class="fqi-title" style="font-weight: 600; color: var(--app-text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                    ${highlight(title, term)}${clientName}
                </span>
            </div>
            <div style="display: flex; gap: 0.4rem;">
                ${src && src !== 'None' && src !== '' ? 
                    `<button type="button" class="btn-primary js-drop-preview" data-src="${src}" data-title="${pTitle}" style="padding: 0.3rem 0.6rem; font-size: 0.75rem;"><i class="fa-solid fa-eye"></i> Ver</button>` 
                    : ''}
                
                ${downloadUrl !== '#' ? 
                    `<a href="${downloadUrl}" class="btn-secondary" style="padding: 0.3rem 0.6rem; font-size: 0.75rem; text-decoration: none;"><i class="fa-solid fa-download"></i> Baixar</a>` 
                    : ''}
            </div>
        </div>
    `;
...
function renderBulkPairs(pairs) {
    const grid = document.getElementById("bulkPairsGrid");
    const confirmBtn = document.getElementById("btnConfirmBulkCreate");
    ...
    pairs.forEach((pair, index) => {
        const isComplete = pair.hasJson && pair.hasImage;
        if (isComplete) validPairs++;

        // Gerar as opções do Select de FAQ
        let optionsHtml = '<option value="">-- Selecione a FAQ Obrigatória --</option>';
        ALL_FAQS.forEach(faq => {
            optionsHtml += `<option value="${faq.id}">${faq.title}</option>`;
        });

        const card = document.createElement("div");
        card.className = `bulk-card ${isComplete ? 'is-complete' : 'is-incomplete'}`;
        card.style = "background: var(--app-bg-card); border: 1px solid var(--app-border); padding: 1rem; border-radius: 8px; display: flex; flex-direction: column; gap: 0.8rem;";

        card.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                <strong style="font-size: 0.9rem; color: var(--app-text); word-break: break-all;">${pair.name}</strong>
                <span style="font-size: 0.7rem; padding: 2px 6px; border-radius: 4px; background: ${isComplete ? '#d1fae5' : '#fee2e2'}; color: ${isComplete ? '#065f46' : '#991b1b'};">
                    ${isComplete ? 'Pronto' : 'Incompleto'}
                </span>
            </div>

            <div class="form-group" style="margin: 0;">
                <label style="font-size: 0.75rem; font-weight: bold; margin-bottom: 4px; display: block;">Vincular FAQ:</label>
                <select class="form-input bulk-faq-select" data-index="${index}" style="font-size: 0.8rem; padding: 0.4rem;" required>
                    ${optionsHtml}
                </select>
            </div>

            <div style="display: flex; gap: 1rem; font-size: 0.75rem; color: var(--app-muted);">
                <span><i class="fa-solid ${pair.hasJson ? 'fa-check' : 'fa-x'}"></i> JSON</span>
                <span><i class="fa-solid ${pair.hasImage ? 'fa-check' : 'fa-x'}"></i> Imagem</span>
            </div>
        `;
        grid.appendChild(card);
    });

    pairCountEl.textContent = `${validPairs} Dashboards Válidos`;
    previewSection.style.display = "block";
    confirmBtn.style.display = validPairs > 0 ? "block" : "none";
}
...
function scrollBulkModalStepIntoView(targetElement, options = { instant: false }) {
  ...
  if (bulkModal && bulkModal.contains(targetElement) && scrollContainer) {
    const targetRect = targetElement.getBoundingClientRect();
    const containerRect = scrollContainer.getBoundingClientRect();
    const currentScroll = scrollContainer.scrollTop;
    const offset = currentScroll + (targetRect.top - containerRect.top) - 120;
    // rolagem instantânea ou suave conforme option
    scrollContainer.scrollTo({ top: Math.max(offset, 0), behavior });
    return;
  }
...
function startBulkTour() {
    const introFactory = typeof introJs === 'function'
        ? introJs
        : (typeof introJs === 'object' && typeof introJs.tour === 'function' ? introJs.tour : null);

    if (typeof introFactory !== 'function') {
        alert('Tour não disponível no momento. Verifique se o intro.js foi carregado corretamente.');
        return;
    }

    const tour = introFactory();
    if (!tour || typeof tour.setOptions !== 'function') {
        alert('Tour não disponível no momento. Verifique se o intro.js foi carregado corretamente.');
        return;
    }

    const hasPairs = bulkDashboardPairs.length > 0;
    const dropZoneEl = document.getElementById('bulkDropZone');
    const previewEl = document.getElementById('bulkPreviewSection');
    const firstFaqSelect = document.querySelector('.bulk-faq-select');
    const confirmBtnEl = document.getElementById('btnConfirmBulkCreate');
    const modalEl = document.getElementById('bulkCreateModal');

    const steps = [
      {
        element: dropZoneEl || '#bulkDropZone',
        title: 'Passo 1: Upload',
        intro: 'Clique ou arraste seus arquivos JSON e as imagens aqui. Cada par deve ter o mesmo nome para ser pareado automaticamente.',
        position: 'bottom'
      }
    ];
    ...
    if (hasPairs) {
      steps.push(
        {
          element: previewEl || '#bulkPreviewSection',
          title: 'Passo 2: Conferência',
          intro: 'Veja os pares criados. Se alguma entrada estiver incompleta, verifique o JSON e a imagem correspondentes.',
          position: 'top'
        },
        {
          element: firstFaqSelect || '#bulkPairsGrid',
          title: 'Passo 3: Seleção de FAQ',
          intro: 'Selecione uma FAQ obrigatória para cada dashboard antes de confirmar a criação.',
          position: 'right'
        },
        {
          element: confirmBtnEl || '#btnConfirmBulkCreate',
          title: 'Passo 4: Finalizar',
          intro: 'Clique aqui para salvar todos os dashboards em massa depois de revisar e selecionar as FAQs.',
          position: 'top'
        }
      );
    } else {
      steps.push({
        element: modalEl || '#bulkCreateModal',
        title: 'Próximo passo',
        intro: 'Envie seus arquivos primeiro para ver os cards de pré-visualização e concluir a criação em massa.',
        position: 'floating'
      });
    }
    ...
    alert('Não foi possível iniciar o tour. Verifique a versão do intro.js carregada.');
}

window.startBulkTour = startBulkTour;