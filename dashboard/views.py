    ...
    if request.method == 'POST':
        try:
            ...
            for i in range(count):
                ...
                # MUDANÇA AQUI: Pegamos a FAQ individual enviada pelo JS
                faq_id = request.POST.get(f'faq_{i}')
                
                json_file = request.FILES.get(f'json_{i}')
                image_file = request.FILES.get(f'image_{i}')

                # VALIDAÇÃO: Agora a FAQ é obrigatória por item
                if titulo and json_file and image_file and faq_id:
                    ...
                    dash = Dashboard(
                        ...
                        faq=faq_obj  # Vincula a FAQ correta
                    )
            ...
            messages.success(request, f'{criados} dashboards criados com sucesso!')
...
def dashboards(request, slug):
    ...
    dashboards_qs = Dashboard.objects.filter(client=cliente).select_related('created_by', 'created_by__perfil', 'faq')
